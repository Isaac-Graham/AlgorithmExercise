...
// 
// Related Topics 数组 动态规划 前缀和 👍 21 👎 0
...
public:
    vector<int> goodDaysToRobBank(vector<int> &security, int time) {
        vector<int> increase(security.size(), 0);
        vector<int> decrease(security.size(), 0);

        for (int i = 1; i < security.size(); i++) {
            if (security[i] <= security[i - 1]) decrease[i] = decrease[i - 1] + 1;
        }
        for (int i = security.size() - 2; i >= 0; i--) {
            if (security[i + 1] >= security[i]) increase[i] = increase[i + 1] + 1;
        }

        vector<int> res;
        for (int i = 0; i < security.size(); i++) {
            if (increase[i] >= time && decrease[i] >= time) res.push_back(i);
        }
        return res;
    }
};
...
int main() {
    ...
    vector<int> res = s->goodDaysToRobBank(vec, 0);
    for (auto i : res) {
        cout << i << endl;
    }
}